...
function initCardAnimations() {
    const cards = gsap.utils.toArray<HTMLElement>('.truus-root .card');
    if (!cards.length) return;

    const originalData = [{ rotation: 4 }, { rotation: -5 }, { rotation: 5 }, { rotation: -8 }, { rotation: 5 }];
    ...
    if (!isMobile) {
        cards.forEach((card, index) => {
            card.addEventListener('mouseenter', () => {
                ...
                const leftCards: Array<{ card: HTMLElement; index: number }> = [];
                const rightCards: Array<{ card: HTMLElement; index: number }> = [];
            ...
            card.addEventListener('mouseleave', () => {
                leaveTimeout = setTimeout(() => {
                    cards.forEach((c, i) => {
                        gsap.to(c, { x: 0, y: 0, scale: 1, rotation: originalData[i].rotation, duration: 1.0, ease: 'elastic.out(1, 0.5)', overwrite: true });
                    });
                }, 80);
            });
        });
    } else {
        const cardsWrapper = document.querySelector('.truus-root .cards-wrapper') as HTMLElement | null;
...
export default function TruusServiceCards() {
    useEffect(() => {
        gsap.to('.truus-root .title-underline-svg path', {
            strokeDashoffset: 0,
            duration: 1.2,
            ease: 'power3.out',
            stagger: 0.3,
            scrollTrigger: {
                trigger: '.truus-root .service-cards-wrapper',
                start: 'top 70%',
                ...
                {CARDS_DATA.map((card) => (
                    <div key={card.color} className={`card card-${card.color}`}>
                        <div className={`card-sticker sticker-${card.sticker}`}>
                            <img src={`/truus-assets/assets/Card-Sticker SVG/sticker-${card.sticker}.svg`} alt="" width="100%" loading="lazy" aria-hidden="true" />
                        </div>
