...
export default function TruusMotionCards() {
    const sectionRef = useRef<HTMLElement>(null);

    useEffect(() => {
        const ctx = gsap.context(() => {
            // Inertia on cards
            const cards = document.querySelectorAll('.truus-root .motion-card__card');
            cards.forEach((card) => {
                let lastX = 0,
                    lastY = 0,
                    speedX = 0,
                    speedY = 0;
                const startRotation = gsap.getProperty(card, 'rotation') as number;
                const startX = gsap.getProperty(card, 'x') as number;
                const startY = gsap.getProperty(card, 'y') as number;
                ...
                const onLeave = () => {
                    gsap.to(card, { inertia: { x: { velocity: speedX * 20, end: startX }, y: { velocity: speedY * 20, end: startY }, rotation: { velocity: speedX * 1.5, end: startRotation } } });
                };
            ...
            const labels = document.querySelectorAll('.truus-root .motion-card__floating-label');
            labels.forEach((label) => {
                let lastX = 0,
                    lastY = 0,
                    speedX = 0,
                    speedY = 0;
                const startRotation = gsap.getProperty(label, 'rotation') as number;
                const startX = gsap.getProperty(label, 'x') as number;
                const startY = gsap.getProperty(label, 'y') as number;
                ...
                const onLeave = () => {
                    gsap.to(label, { inertia: { x: { velocity: speedX * 25, end: startX }, y: { velocity: speedY * 25, end: startY }, rotation: { velocity: speedX * 2, end: startRotation } } });
                };
            ...
            const underlinePath = sectionRef.current?.querySelector('.motion-card__underline-path') as SVGPathElement | null;
            if (underlinePath) {
        ...
        <section ref={sectionRef} className="motion-card-section" id="motion-card-section">
            <div className="motion-card__heading">
                ...
                <p className="motion-card__subtitle">
                    ...
                    <span className="motion-card__sticker motion-card__sticker--top">
                        <img src="/truus-assets/assets/Footer-Sticker SVG/footer-sticker-hands.svg" alt="Green heart hands sticker" className="motion-card__sticker-img" />
                    </span>
                </p>
            ...
            <div className="motion-card__cards-area">
                <div className="motion-card__blob">
                    <img src="/truus-assets/assets/MotionCard SVG/motion-card-blob.svg" alt="" className="motion-card__blob-svg" />
                </div>
            ...
            <div className="motion-card__footer-text">
                <p className="motion-card__description">
                    To reach the new generation you need to know where they are. We are a true 360° agency, working the whole spectrum from TikTok content to TVC and from influencer collabs to out of
                    home spectaculars.
                </p>
            </div>
        </section>
