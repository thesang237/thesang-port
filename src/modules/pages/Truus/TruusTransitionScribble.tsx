...
export default function TruusTransitionScribble() {
    useEffect(() => {
        const logoTruus = document.querySelector('.truus-root .logo-truus');
        const transitionScribblePath = document.querySelector('.truus-root .transition-scribble path') as SVGPathElement | null;
        const transitionScribbleSvg = document.querySelector('.truus-root .transition-scribble') as SVGElement | null;
        const truusRoot = document.querySelector('.truus-root') as HTMLElement | null;

        if (!logoTruus || !transitionScribblePath || !transitionScribbleSvg || !truusRoot) return;
        ...
        const runScribbleAnimation = (e: Event | null) => {
            ...
            if (gsap.isTweening(transitionScribblePath) || gsap.isTweening(transitionScribbleSvg) || truusRoot.classList.contains('is-transitioning')) return;
            ...
            let transitionLogo = truusRoot.querySelector('.transition-logo') as HTMLElement | null;
            if (!transitionLogo) {
                transitionLogo = document.createElement('div');
                transitionLogo.className = 'transition-logo';
                transitionLogo.style.cssText =
                    'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:10000;pointer-events:none;opacity:0;display:flex;justify-content:center;align-items:center;';
                const svgClone = logoTruus.cloneNode(true) as SVGElement;
                svgClone.style.width = '150px';
                svgClone.style.height = 'auto';
                transitionLogo.appendChild(svgClone);
                truusRoot.appendChild(transitionLogo);
            }
            ...
            truusRoot.classList.add('is-transitioning');
            const cursorBubble = truusRoot.querySelector('.cursor-bubble');
            if (cursorBubble) gsap.to(cursorBubble, { opacity: 0, duration: 0.2 });

            const drawTl = gsap.timeline({
                onComplete: () => {
                    truusRoot.classList.remove('is-transitioning');
            ...
            drawTl.to(
                transitionLogo,
                {
                    ...
                    ease: 'power2.out',
                    onStart: () => {
                        const innerSvg = transitionLogo!.querySelector('svg');
                        if (innerSvg) gsap.to(innerSvg, { rotation: 5, duration: 0.15, repeat: -1, yoyo: true, ease: 'steps(1)', overwrite: 'auto' });
                    },
                },
                durIn * 0.5,
            );
            ...
            drawTl.set(
                transitionLogo,
                {
                    ...
                    onComplete: () => {
                        const innerSvg = transitionLogo!.querySelector('svg');
                        if (innerSvg) {
                            gsap.killTweensOf(innerSvg);
                            gsap.set(innerSvg, { rotation: 0 });
                        }
                    },
                },
                durIn + durOut * 0.48,
        ...
        logoTruus.addEventListener('click', runScribbleAnimation);
        const timer = setTimeout(() => runScribbleAnimation(null), 100);

        return () => {
            logoTruus.removeEventListener('click', runScribbleAnimation);
            clearTimeout(timer);
        };
    }, []);

    return (
        <svg xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 3222 3114" fill="none" preserveAspectRatio="none" className="transition-scribble">
            <path
