...
import styles from './frosted-border.module.css';

import { useCallback, useEffect, useRef, useState } from 'react';

// ─── Data ─────────────────────────────────────────────────────────────────────
const IMAGE_COUNT = 50;
const IMAGES = Array.from({ length: IMAGE_COUNT }, (_, i) => `https://picsum.photos/400/300?random=${i + 1}`);
...
// ─── Page ──────────────────────────────────────────────────────────────────────
export default function FrostedBorderPage() {
    const [theme, setTheme] = useState<Theme>('system');
    const [disable, setDisable] = useState(false);
    const asideRef = useRef<HTMLElement>(null);
    const dragState = useRef({ dragging: false, startX: 0, startY: 0, x: 0, y: 0 });

    // ── Drag handling (replaces GSAP Draggable) ────────────────────────────────
    const onPointerDown = useCallback((e: React.PointerEvent) => {
        const aside = asideRef.current;
        if (!aside) return;
        const ds = dragState.current;
        ds.dragging = true;
        ds.startX = e.clientX - ds.x;
        ds.startY = e.clientY - ds.y;
    ...
    const onPointerMove = useCallback((e: React.PointerEvent) => {
        ...
        const aside = asideRef.current;
        if (!aside) return;
        ds.x = e.clientX - ds.startX;
        ds.y = e.clientY - ds.startY;
        aside.style.translate = `${ds.x}px ${ds.y}px`;
    }, []);
    ...
    const onDoubleClick = useCallback(() => {
        ...
        ds.x = 0;
        ds.y = 0;
        aside.style.transition = 'translate 0.35s cubic-bezier(0.22, 1, 0.36, 1)';
        aside.style.translate = '0px 0px';
        const cleanup = () => {
            aside.style.transition = '';
            aside.removeEventListener('transitionend', cleanup);
        };
        aside.addEventListener('transitionend', cleanup);
    }, []);

    // ── Load Google Font for the annotation ────────────────────────────────────
    useEffect(() => {
        const id = 'gloria-hallelujah-font';
    ...
    return (
        <div className={styles.page} data-theme={theme === 'system' ? undefined : theme} data-disable={String(disable)} style={configVars}>
            <header />

            <main className={styles.main}>
                {/* ── Product card (draggable, sticky, frosted) ── */}
                <aside ref={asideRef} className={styles.product} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onDoubleClick={onDoubleClick}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img className={styles.productImg} src="https://assets.codepen.io/605876/the-cap-boardwalk.png" alt="" draggable={false} />
                    <div className={styles.productInfo}>
                        <span className={styles.productName}>Cap 001</span>
                        <div>
                            <span>
                                Drag me • <s className={styles.strikethrough}>$30</s> $23
                            </span>
                        </div>
                    </div>
            ...
            <div className={styles.configBar}>
                <button className={styles.configBtn} data-active={String(disable)} onClick={() => setDisable((d) => !d)}>
                    {disable ? 'effect off' : 'effect on'}
                </button>
